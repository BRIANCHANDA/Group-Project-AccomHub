...
const PropertyCreationForm = ({ isOpen, onClose, onSubmit, landlordId }: PropertyCreationFormProps) => {
      ...
      try {
        ...
        setPropertyData(prev => ({
          ...
          images: [...prev.images, {
            file,
            preview: previewUrl,
            isPrimary: prev.images.length === 0 && prev.images.length === 0
          }]
        }));
      } catch (error) {
  ...
  // Upload images to server - now the server will handle the base64 conversion
  const uploadImages = async (propertyId: string): Promise<boolean> => {
    try {
      ...
      console.log('Uploading images with propertyId:', propertyId); // Add debug log
      ...
      for (let i = 0; i < totalImages; i++) {
        ...
        try {
          ...
          formData.append('propertyId', propertyId); // Ensure propertyId is correctly formatted
          ...
          if (!imageRes.ok) {
            ...
            console.error('Image upload response:', await imageRes.clone().text()); // Debug response
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const propertyId = draftData.propertyId;
      
      // Add explicit debugging
      console.log('Draft property created with ID:', propertyId);
      console.log('Type of propertyId:', typeof propertyId);
  
      // Step 2: Update the draft property with complete information
      const updateRes = await fetch(`/api/properties/properties/${propertyId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Step 3: Upload images - ensure propertyId is correctly formatted
      console.log('About to upload images with property ID:', propertyId);
      const propertyIdStr = String(propertyId);
      console.log('propertyId as string:', propertyIdStr);
      
      const imagesUploaded = await uploadImages(propertyIdStr);
      if (!imagesUploaded) {
        try {
          await fetch(`/api/properties/properties/${propertyId}`, {
            method: 'DELETE'
          });
        } catch (deleteError) {
          console.error('Failed to cleanup property after image upload failure:', deleteError);
        }
        
        throw new Error('Failed to upload images. Property creation cancelled.');
      }
  
      
 
  
      // Step 4: Update Property Details
      try {
        const detailsRes = await fetch(`/api/property-details/properties/${propertyId}/details`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        alert('Property created successfully with images, but there was an issue updating additional details. You can edit these later.');
      }
